...
class CCVID_VIDEO(object):
    ...
    def __init__(self, root='/data/datasets/', sampling_step=64, seq_len=16, stride=4, **kwargs):
        ...
        logger.info("  recombined_query        | {:5d} | {:11d} | {:9d}".format(num_query_pids, len(recombined_query), num_query_clothes))
        logger.info("  recombined_gallery      | {:5d} | {:11d} | {:9d}".format(num_gallery_pids, len(recombined_gallery), num_gallery_clothes))
        logger.info("  ---------------------------------------------")
        ...
        self.num_camera=24
        self.num_query_imgs = num_query_tracklets
        self.num_gallery_imgs = num_gallery_tracklets
    ...
    def _densesampling_for_trainingset(self, dataset, sampling_step=64):
        ...
        new_dataset = []
        for data in dataset:
            #(img_paths, pid, camid, clothes_id)=data
            img_paths=data['image_path'].copy()
            if sampling_step != 0:
                ...
                if num_sampling == 0:
                    ...
                    for idx in range(num_sampling):
                        if idx == num_sampling - 1:
                            ...
                            data['image_path']=img_paths[idx*sampling_step:]
                            new_dataset.append(data)
                        else:
                            #new_dataset.append((img_paths[idx*sampling_step : (idx+1)*sampling_step], pid, camid, clothes_id))
                            data['image_path']=img_paths[idx*sampling_step : (idx+1)*sampling_step]
                            new_dataset.append(data)
            else:
                #new_dataset.append((img_paths, pid, camid, clothes_id))
                data['image_path']=img_paths
        ...
        for idx, data in enumerate(dataset):
            ...
            for i in range(len(img_paths)//(seq_len*stride)):
                for j in range(stride):
                    ...
                    newdata=data.copy()
                    newdata['image_path']=clip_paths
                    new_dataset_idx.append(newdata)
            ...
            if len(img_paths)%(seq_len*stride) != 0:
                ...
                if len(img_paths) % seq_len != 0:
                    ...
                    newdata['image_path']=clip_paths
                    new_dataset_idx.append(newdata)
            new_dataset_idx_reduecesize=new_dataset_idx[0:len(new_dataset_idx):8]
            new_dataset=new_dataset+new_dataset_idx_reduecesize
            # end index
